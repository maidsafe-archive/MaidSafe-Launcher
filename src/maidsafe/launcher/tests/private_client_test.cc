...
#include "maidsafe/launcher/private_client.h"
...
TEST(PrivateClientTest, FUNC_CreateValidAccount) {
  ...
  std::future<std::unique_ptr<PrivateClient>> private_client_future{PrivateClient::CreateAccount(
...
TEST(PrivateClientTest, FUNC_CreateDuplicateAccount) {
  ...
  }
  {
    ...
    EXPECT_TRUE(ThrowsAs([&] { private_client_future.get(); }, VaultErrors::data_already_exists));
  }
}
...
TEST(PrivateClientTest, FUNC_ValidLogin) {
  ...
  }
  {
    ...
    std::unique_ptr<PrivateClient> private_client;
    std::future<std::unique_ptr<PrivateClient>> private_client_future{PrivateClient::Login(
        std::get<0>(user_credentials_tuple), std::get<1>(user_credentials_tuple),
        std::get<2>(user_credentials_tuple))};
    ASSERT_NO_THROW(private_client = private_client_future.get());
    private_client->Logout();
